...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <motion.header
        className="fixed left-0 top-0 h-screen w-64 bg-white/80 backdrop-blur-xl border-r border-stone-200/50 shadow-sm shadow-black/5 z-[100]"
        initial={{ x: -256, opacity: 0 }}
        animate={{ x: 0, opacity: 1 }}
        transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
      >
        <nav className="flex flex-col items-start py-6 px-4 gap-4 h-full">
          {/* Logo */}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            className="fixed inset-0 z-40 bg-white/96 backdrop-blur-xl flex flex-col items-center justify-center gap-7 md:hidden"
