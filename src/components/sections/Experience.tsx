...
const typeColors: Record<string, string> = {
  Leadership: "text-stone-700 bg-stone-100 border-stone-200",
  Entrepreneurship: "text-stone-700 bg-stone-100 border-stone-200",
};
    ...
    <section id="experience" className="py-28 px-6 sm:px-10">
      <div className="max-w-7xl mx-auto">
        <motion.div className="flex items-center gap-3 mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <span className="text-xs font-mono text-stone-500 tracking-widest uppercase">02</span>
        </motion.div>
                    ...
                    <div className="glass-card rounded-2xl p-5 shadow-xl shadow-black/20">
                      <div className="flex flex-wrap items-start justify-between gap-2 mb-3">
                        <div>
                          ...
                          <p className="text-sm text-stone-500 mt-0.5">{exp.company}</p>
                        </div>
                        <div className="flex flex-col items-end gap-1.5">
                          <span className="text-[10px] font-mono text-stone-500">{exp.duration}</span>
                          <span className={`text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 rounded-full border ${typeColors[exp.type] ?? "text-stone-500 bg-stone-50 border-stone-100"}`}>{exp.type}</span>
                        </div>
                      </div>
                      <ul className="space-y-2">
                        {exp.description.map((d, j) => (
                          <li key={j} className="flex gap-2.5 text-sm text-stone-600 leading-relaxed">
                            <span className="text-stone-400 mt-1.5 flex-shrink-0 text-xs">▹</span>{d}
                          </li>
                        ))}
                      </ul>
                    </div>
